...
def obtener_contexto_gratuito(local, visitante):
    ...
    try:
        ...
        return f"📊 RACHAS RECIENTES (PD/SD):\n- {local}: {r_l}\n- {visitante}: {r_v}\n(W=Gana, D=Empate, L=Pierde)" if r_l and r_v else "No se hallaron rachas."
...
async def guardar_en_historial_github(partido, analisis, pick_ia):
    ...
    try:
        ...
        content.append({
            ...
            "partido": partido, "pick_pronosticado": pick_ia,
            "analisis_resumen": analisis[:200] + "...", "resultado_real": "Pendiente"
        })
        new_b64 = base64.b64encode(json.dumps(content, indent=4).encode('utf-8')).decode('utf-8')
        payload = {"message": f"Log: {partido}", "content": new_b64, "sha": file_data['sha']}
...
async def handle_analisis(message):
    ...
    header_checks = f"🛠 **REPORTE DE DATOS:**\n✅ Poisson | ✅ Rachas | {'✅' if cuotas_data else '❌'} Cuotas\n──────────────────────────────\n\n"
...
async def cmd_historial(message):
    try:
        ...
        if r.status_code == 200:
            ...
            for l in logs: texto += f"• `{l['fecha']}`: **{l['partido']}** | Pick: *{l.get('pick_pronosticado', 'N/A')}*\n"
